...
-- some more things
  ...
  cup_of_CN    np = mkCN (lin N2 (mkN2 (mkN "taça") part_Prep)) np ;
  glass_of_CN  np = mkCN (lin N2 (mkN2 (mkN "copo") part_Prep)) np ; -- copa

{-
-- spatial deixis and motion verbs

  where_go_QCl np = mkQCl where_IAdv (mkCl np (mkVP L.go_V)) ;
  where_come_from_QCl np =  mkQCl (lin IAdv (ss "de onde")) (mkCl np (mkVP L.go_V)) ;
  
  go_here_VP = mkVP (mkVP L.go_V) here_Adv ;
  come_here_VP = mkVP (mkVP L.come_V) here_Adv ;
  come_from_here_VP = mkVP (mkVP L.come_V) (mkAdv "daqui") ;

  go_there_VP = mkVP (mkVP L.go_V)  there_Adv ;
  come_there_VP = mkVP (mkVP L.come_V) there_Adv ;
  come_from_there_VP = mkVP (mkVP L.come_V) (mkAdv "dali") ; -- "de lá"
-}  
...
lin
  ...
  monthAdv m = lin Adv {s = "em" ++ m.s ! C.Sg} ;         -- in mggio
...
lin estonian_Language = mkLanguage "estoniano" ;
...
lin spanish_Language = mkLanguage "espanhol" | mkLanguage "castellano" ;
